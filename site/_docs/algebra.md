...
{% assign sourceRoot = "http://github.com/julianhyde/incubator-calcite/blob/748-builder" %}
...
[RelBuilder](/apidocs/org/apache/calcite/tools/RelBuilder.html).
Here is an example:
...
([RelOptRule](/apidocs/org/apache/calcite/plan/RelOptRule.html)), the field
...
| `filter(expr...)`<br/>`filter(exprList)` | Creates a [Filter](/apidocs/org/apache/calcite/rel/core/Filter.html) over the AND of the given predicates.
...
| `aggregate(groupKey, aggCall...)`<br/>`aggregate(groupKey, aggCallList)` | Creates an [Aggregate](/apidocs/org/apache/calcite/rel/core/Aggregate.html).
| `distinct()` | Creates an [Aggregate](/apidocs/org/apache/calcite/rel/core/Aggregate.html) that eliminates duplicate records.
...
| `join(joinType, expr)`<br/>`join(joinType, fieldName...)` | Creates a [Join](/apidocs/org/apache/calcite/rel/core/Join.html) of the two most recent relational expressions.<br/><br/>The first form joins on an boolean expression.<br/><br/>The second form joins on named fields; each side must have a field of each name.
...
* `aggCall...` Array of [RelBuilder.AggCall](/apidocs/org/apache/calcite/tools/RelBuilder/AggCall.html)
* `aggCallList` Iterable of [RelBuilder.AggCall](/apidocs/org/apache/calcite/tools/RelBuilder/AggCall.html)
