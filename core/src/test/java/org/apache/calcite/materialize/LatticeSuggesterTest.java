...
public class LatticeSuggesterTest {
  ...
  @Test public void testAggregateExpression() throws Exception {
    ...
    final String q = "select \"t\".\"the_year\" as \"c0\",\n"
        ...
        + " \"pc\".\"product_family\"\n";
    assertThat(t.addQuery(q),
        isGraphs()); // no graphs because cannot handle agg(expr) yet
  }
  ...
  private static class Tester {
    ...
    Tester() {
      this(config(CalciteAssert.SchemaSpec.SCOTT).build());
    }
    ...
    Tester foodmart() {
      return withConfig(config(CalciteAssert.SchemaSpec.JDBC_FOODMART).build());
    }
