...
public class SqlParserTest {
  ...
  @Test public void testSequence() {
    sql("select next value for my_schema.my_seq from t")
        .ok("SELECT NEXT_VALUE(`MY_SCHEMA`.`MY_SEQ`)\n"
                + "FROM `T`");
    sql("select next value for my_schema.my_seq as s from t")
        .ok("SELECT NEXT_VALUE(`MY_SCHEMA`.`MY_SEQ`) AS `S`\n"
                + "FROM `T`");
    sql("select next value for my_seq as s from t")
        .ok("SELECT NEXT_VALUE(`MY_SEQ`) AS `S`\n"
                + "FROM `T`");
    sql("select 1 + next value for my_seq + current value for my_seq from t")
        .ok("SELECT ((1 + NEXT_VALUE(`MY_SEQ`)) + CURRENT_VALUE(`MY_SEQ`))\n"
                + "FROM `T`");
    sql("select 1 from t where next value for my_seq < 10")
        .ok("SELECT 1\n"
                + "FROM `T`\n"
                + "WHERE (NEXT_VALUE(`MY_SEQ`) < 10)");
    sql("select 1 from t\n"
    ...
    sql("insert into t values next value for my_seq, current value for my_seq")
        .ok("INSERT INTO `T`\n"
                + "(VALUES (ROW(NEXT_VALUE(`MY_SEQ`))), (ROW(CURRENT_VALUE(`MY_SEQ`))))");
    sql("insert into t values (1, current value for my_seq)")
        .ok("INSERT INTO `T`\n"
                + "(VALUES (ROW(1, CURRENT_VALUE(`MY_SEQ`))))");
  }
